...
type Relation = { from:string; to:string; name:string; column:string; targetColumn:string; status:string };
...
export function SchemaExplorer({ onBack }:{ onBack:()=>void }) {
  const [query,setQuery]=useState(''); const [selected,setSelected]=useState(''); const [depth,setDepth]=useState(1); const [domain,setDomain]=useState(''); const [tab,setTab]=useState('Details'); const [expanded,setExpanded]=useState<Record<string,boolean>>({}); const [dataset,setDataset]=useState(''); const [zoom,setZoom]=useState(1);
  ...
  const centreView=()=>{const board=boardRef.current;if(board){board.scrollTo({left:Math.max(0,(board.scrollWidth-board.clientWidth)/2),top:Math.max(0,(board.scrollHeight-board.clientHeight)/2),behavior:'smooth'})}};
  const fitView=()=>{setZoom(1);requestAnimationFrame(()=>boardRef.current?.scrollTo({left:0,top:0,behavior:'smooth'}))};
  const toggleFullscreen=async()=>{if(document.fullscreenElement)await document.exitFullscreen();else await canvasRef.current?.requestFullscreen()};
  const beginPan=(e:React.PointerEvent<HTMLDivElement>)=>{if(e.button!==0||(e.target as HTMLElement).closest('button,.sx-table,.sx-map-table,.sx-link-label'))return;const board=e.currentTarget;dragRef.current={active:true,x:e.clientX,y:e.clientY,left:board.scrollLeft,top:board.scrollTop};board.setPointerCapture(e.pointerId);board.classList.add('is-panning')};
  const movePan=(e:React.PointerEvent<HTMLDivElement>)=>{const drag=dragRef.current;if(!drag.active)return;const board=e.currentTarget;board.scrollLeft=drag.left-(e.clientX-drag.x);board.scrollTop=drag.top-(e.clientY-drag.y)};
  const endPan=(e:React.PointerEvent<HTMLDivElement>)=>{dragRef.current.active=false;e.currentTarget.classList.remove('is-panning');if(e.currentTarget.hasPointerCapture(e.pointerId))e.currentTarget.releasePointerCapture(e.pointerId)};
...
function RelationshipMapV2({table,relations:onMap,onSelect,onClose}:{table:Table;relations:Relation[];onSelect:(name:string)=>void;onClose:()=>void}) {
  const allSlots=[
    {side:'left',y:10,labelY:62,path:'M400 270H335V100H200'},
    {side:'right',y:10,labelY:62,path:'M600 270H665V100H800'},
    {side:'left',y:210,labelY:272,path:'M400 310H200'},
    {side:'right',y:210,labelY:272,path:'M600 310H800'},
    {side:'left',y:410,labelY:462,path:'M400 350H335V500H200'},
    {side:'right',y:410,labelY:462,path:'M600 350H665V500H800'},
  ];
  ...
  return <section className="sx-schema-map"><button className="sx-all-tables" onClick={onClose}>← All tables</button><svg viewBox="0 0 1000 620" preserveAspectRatio="none" aria-hidden="true"><defs><marker id="schema-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0 10 5 0 10Z"/></marker></defs>{shown.map((r,i)=><path key={r.name} className={r.status} d={slots[i].path}/>)}</svg><SchemaTableCard table={table} central onSelect={onSelect}/>{shown.map((r,i)=><div className={`sx-link-label ${slots[i].side} ${r.status}`} style={{top:slots[i].labelY}} title={`${r.name}: FK ${r.from}.${r.column} → PK ${r.to}.${r.targetColumn||'key'}`} key={`${r.name}-label`}><em>{r.status==='inferred'?'Inferred':'Confirmed'}</em><span><b>FK</b><code>{r.column}</code><i>→</i><b className="pk">PK</b><code>{r.targetColumn||'key'}</code></span></div>)}{shown.map((r,i)=>{const other=r.from===table.name?r.to:r.from;const otherTable=tables.find(t=>t.name===other);return otherTable?<div className={`sx-map-node ${slots[i].side}`} style={{top:slots[i].y}} key={r.name}><SchemaTableCard table={otherTable} relation={r} onSelect={onSelect}/></div>:null})}{!onMap.length&&<p className="sx-no-links">No validated foreign keys were found for {table.name}.</p>}</section>
}
